...
#========================================
# we still need to use tempo2 to adjust other oarameters and fit for them in tempo2
#========================================
...
echo -e "\n Start updating the obs with the new parfile and dm and then scrunch the data to F8T1"
...
# Start tempo2 to check and adjust the parameters  for the second time to inhnce the precission of the prameters
#===================
 #this step is cancelled, there is no significant improvements
#===================
#echo -e "\n Check and adjust the parameters in the initial parfile and get new_parfil.par"
...
tail -n +2 $dir/Second_loop/TOAs.tim | grep -v "C "| cut -d " " -f4 | cut -d "." -f1 | awk '{print ($1)" "($1)+1" a a"}' > strides0.dat
uniq -c $dir/Second_loop/strides0.dat  | awk '{ print $2" " $3" " $4 " " $5}' > strides.dat
tempo2 -gr dm -t $dir/Second_loop/strides.dat -npsr 1 -nobs 300000 -f $dir/Second_loop/new_parfile.par $dir/Second_loop/TOAs.tim
